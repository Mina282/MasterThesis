...
model_basename = 'Styled_S3_' # name of the "experiment", used for saving the models/results
txt_out_name = 'S3_BP30_'  # nametag to add to each of the generated .txt files Styled_S3_gbook_4_ Styled_S3_OODseed_20_
...
def create_model(feed_forward_dim=feed_forward_dim,n_layers=n_layers):
    inputs = layers.Input(shape=(maxlen+1,), dtype=tf.int32)
...
def format_training_data(text_ds,ages,en_tokenizer,maxlen):        
    ...
    # Determine prefix for each row based on A
    # 8001 if out_of_range_age exists in row, else 8000
    prefixes = np.where(np.any(A == out_of_range_age, axis=1), vocab.index("[style2]"), vocab.index("[style1]"))
    
    # Add prefix column to X and Y
    X = np.hstack((prefixes.reshape(-1, 1), X))
    Y = np.hstack((prefixes.reshape(-1, 1), Y))
...
def sample_new(start_tokens,max_tokens,en_tokenizer,styling_token,temperature=1,age_cond=5):
    ...
    while num_tokens_generated <= max_tokens:
        ...
        x = np.insert(x, 0, styling_token) #added
        sample_index +=1
...
def assign_random_value_(target_array: np.ndarray, age_groups : np.ndarray) -> np.ndarray:
    """
    Replaces values of 250 in target_array with random values from age_groups.
    
    Parameters:
        target_array (np.ndarray): The target array where values will be replaced.
        age_groups (np.ndarray): The source array from which random values are chosen.
    
    Returns:
        np.ndarray: The modified target array.
    """
    # Find indices where target_array is 250
    indices = np.where(target_array == out_of_range_age)

    # Assign random values from source_array to those indices
    target_array[indices] = np.random.choice(age_groups) #, size=len(indices[0])

    return target_array


def replace_values(target_array, target_age=dev_age_bin):
    ...
    replacements = np.array([target_age for _ in range(np.sum(mask))], dtype=np.float64)
...
#combined_age_dev= [dev_age_bin]*len(combined_text_dev)


# Tokenize training data
bert_tokenizer_params=dict(lower_case=True)
reserved_tokens = ["[PAD]", "[UNK]", "[START]", "[END]", "[style1]", "[style2]"]
...
vocab_pathname = maindir + 'token_vocabularies/en_vocab_ds_s3_{}.txt'.format(vocab_size)
...
# Remove excess data of book dev:
X_dev_combined = X_dev_combined[:2*len(C_dev)]
Y_dev_combined = Y_dev_combined[:2*len(C_dev)]
C_dev_combined_ = C_dev_combined_[:2*len(C_dev)] 
...
# Assign different random age to C_train_combined_ whenever the values is out of range age                                                                      
...
if(train_model):
    history = model.fit([C_train_combined, X_train_combined], Y_train_combined,validation_data=([C_dev_combined, X_dev_combined], Y_dev_combined), batch_size=batch_size, shuffle=True, verbose=1, epochs=max_epochs, callbacks=[text_gen_callback, tensorboard_callback, earlystop_callback, model_checkpoint_callback])
...
for age_cond in ages_to_synth:    
    tf.keras.backend.clear_session()
    if(generate_book):
        #valid_inds = valid_inds_book
        valid_inds,b = np.where((C_train_combined_ > age_groups[-1])) 
        styling_token = vocab.index("[style2]")
    else:
        #valid_inds = valid_inds_childes
        valid_inds,b = np.where(C_train_combined_ == out_of_range_age) #np.where(C_train_combined_ == out_of_range_age)generate out of domain seed, noramlly: np.where(C_train_combined_ != out_of_range_age)
        styling_token = vocab.index("[style1]")
    for r in range(0,samples_to_generate):        
        ...
        while txt.strip()=="":
            ...
            i = np.random.randint(1,len(element)-start_length) #changed 0 to 1
            start_tokens = np.array(element, dtype=np.int32)
            txt = sample_new(start_tokens[i:i+start_length].tolist(),tokens_to_generate_per_sampling,en_tokenizer,styling_token,sampling_temperature,np.array(age_cond))
