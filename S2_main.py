...
#os.environ["CUDA_VISIBLE_DEVICES"]="0"  # 0 or 1 for GPU selection

print(datetime.datetime.now())

# Setup/experiment name definitions
model_basename = 'S2' # name of the "experiment", used for saving the models/results
txt_out_name = 'S2_BP30_'  # nametag to add to each of the generated .txt files  S2_OODseed_20_'  S2_gbook_4_ 
...
age_groups = [3,6,9,12,15,18,21,24,27,30,33,36,39,42,45,48,51,54,60,63,66,69,72,75,78,84] # all age bins
out_of_range_age = 250
sampling_temperature = 1                # default = 1. Higher means more varied generation.
use_ageseed = 1                         # Sample text using age-bin dependent seed (0/1)?
seed_maxlength = 30                     # Max length of seed prompt used to start generation
generate_book = 0                       # 0 to generate in CDS style, 1 to generate in book text style

# Specify locations for experimental scripts (maindir), location of AO-CHILDES (datadir) and logging (log-dir)

if(os.path.isdir('/home/shahmora/')):
    maindir = '/home/shahmora/narvi_folder/giles_trans/GILES_transcripts/'
    datadir = "/home/shahmora/narvi_folder/giles_trans/GILES_transcripts/aochildes2021_texts/"
    bookdatadir = "/home/shahmora/narvi_folder/giles_trans/GILES_transcripts/children_books/"
    log_dir = "/home/shahmora/narvi_folder/giles_trans/GILES_transcripts/Logs/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
else:
    maindir = 'D:/TUT/07/Thesis/Narvi files/'
    datadir = "D:/TUT/07/Thesis/Narvi files/aochildes2021_texts/"
    bookdatadir = "D:/TUT/07/Thesis/Narvi files/children_books/"
    log_dir = "D:/TUT/07/Thesis/Narvi files/Logs/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
...
def create_model(feed_forward_dim=feed_forward_dim,n_layers=n_layers):
    ...
    inputs1 = layers.Input(shape=(2,), dtype=tf.int32)
    inputs1 = keras.layers.Reshape((1,2))(inputs1)
    inputs2 = layers.Input(shape=(1,), dtype=tf.float32)
    inputs2 = keras.layers.Reshape((1,1))(inputs2)
    embedding_layer = TokenAndPositionEmbedding(maxlen, vocab_size, embed_dim)
    embedding_layer1 = keras.layers.Dense(embed_dim,activation='relu')
    ...
    x = tf.keras.layers.Concatenate(axis=-2)([x2,x1,x])    
    ...
    model = keras.Model(inputs=[inputs2,inputs1,inputs], outputs=[outputs[:,2:,:], x])
...
def sample_new(start_tokens,max_tokens,en_tokenizer,label,temperature=1,age_cond=5):
    ...
    while num_tokens_generated <= max_tokens:
        ...
        third_input = np.array(np.eye(2, dtype=int)[label]).reshape(1, 1, 2)
        y, _ = model.predict([second_input,third_input,x])
...
def replace_values(target_array):
    # Create a copy to avoid modifying the original array
    result = target_array.copy()

    # Flatten the array to make element-wise comparisons easier
    mask = result.flatten() > age_groups[-1]  

    # Generate the same number of random choices as the count of `True` in mask
    replacements = np.array([dev_age_bin for _ in range(np.sum(mask))], dtype=np.float64)

    # Apply replacements while preserving the original shape
    result[mask] = replacements.reshape(-1, 1)
    
    return result

# Assign random age bin value to the age file train and test
def assign_random_values(target_array, weight):
    if not age_groups:
    ...
    # Generate the same number of random choices as the count of `True` in mask
    replacements = np.array([random.choices(age_groups, weights=weight)[0] for _ in range(np.sum(mask))], dtype=np.float64)
...
def format_d(C_trained_combined_,a=0, b=1):
    # Create a copy to avoid modifying the original array
    result = C_trained_combined_.copy()

    # Flatten the array to make element-wise comparisons easier
    mask_greater = C_trained_combined_.flatten() > age_groups[-1]
    mask_less = C_trained_combined_.flatten() <= age_groups[-1]

    # Generate replacements
    replacements_greater = np.array([b for _ in range(np.sum(mask_greater))], dtype=np.float64)
    replacements_less = np.array([a for _ in range(np.sum(mask_less))], dtype=np.float64)

    # Apply replacements while preserving the original shape
    result[mask_greater] = replacements_greater.reshape(-1, 1)
    result[mask_less] = replacements_less.reshape(-1, 1)
    
    result_flat = result.reshape(len(result)).astype(int)
    return result_flat

def one_hot_encode(labels, num_classes=2):
    """Convert a list of class labels to one-hot encoding."""
    one_hot = np.zeros((len(labels), num_classes))  # Create a zero matrix
    one_hot[np.arange(len(labels)), labels] = 1  # Set 1 at the correct index
    
    return one_hot.reshape(len(labels), 1, num_classes)

    
  # TODO more more more :D  
def clean_up_text (text_):
    # Define all the patterns to remove
    patterns = [
        #r"_BOOK_TITLE_.*?out",  # Text starting with "_BOOK_TITLE_" and ending with "out"
                                                                                 
        #r"-LSB-\s+.*?\-RSB-",  # Text starting with -LSB- followed by some characters and ending with -RSB-
        r"-LSB- Illustration :",# texts of value: "-LSB- Illustration :"
        r"CHAPTER\s+.*?\s",     # text of Chapter followed by a white space, next word and again a space
                                                                                          
        r"p.*?\.jpg",           # Any text starting with "p" and ending with ".jpg"
        r"-...-",               # Text starting with "-" and exactly 3 characters in between and ending with "-"
        r"\*\s",                # start followed by a white space
        r"``",
        r"\`\s",
        r"\s'",
        r"''",
...
for file_name in filenames_dev:
    ...
    file_path = file_name
    with open(file_path, 'r') as f: #changed 'r' to 'rb'
        content = f.read()
...
##### START BOOK PART OF PIPELINE

# Read book data from the folder

# List of specific words to check in filenames
keywords = ["test", "train", "valid"]

# Create TensorFlow datasets for data tokenization and assign random age to different parts of it
booktext_ds, booktext_ds_dev = [], []
#book_ages_train, book_ages_dev = [], []

# Iterate through each file in the datadir
for file_name in os.listdir(bookdatadir):
    if file_name.endswith(".txt"):  # Ensure it's a text file
        for keyword in keywords:
            contents =[]
            if keyword in file_name:  # Check if the keyword is in the filename
                if keyword == "test":
                    with open(os.path.join(bookdatadir, file_name), 'r') as file:
                        content = file.read()
                        content = clean_up_text(content)
                        contents = re.split(r"_BOOK_TITLE_.*?out", content)
                    content_list.extend(contents)
                    booktext_ds.extend(contents)
                elif keyword == "train":  
                    with open(os.path.join(bookdatadir, file_name), 'r') as file:
                        content = file.read()
                        content = clean_up_text(content)
                    ...
                    with open(os.path.join(bookdatadir, file_name), 'r') as file:
                        ...
                        content = clean_up_text(content)
                        contents = re.split(r"_BOOK_TITLE_.*?out", content)
                    booktext_ds_dev = np.array(contents)


booktext_ds = np.array(booktext_ds)

book_ages_train = [out_of_range_age]*len(booktext_ds)
book_ages_dev = [out_of_range_age]*len(booktext_ds_dev)


# Combine text for tokenizer use
comibned_text = content_list #np.array(content_list)  #content_list

# combined datasets
combined_age_train = ages_train.copy()
combined_age_train.extend(book_ages_train)

combined_text_dev = np.concatenate([text_ds_dev, booktext_ds_dev]) #np.ma.concatenate((text_ds_dev, booktext_ds_dev), axis=None)
combined_age_dev = np.concatenate([ages_dev, book_ages_dev])
...
vocab_pathname = maindir + 'token_vocabularies/en_vocab_ds_s2_{}.txt'.format(vocab_size)
if(exists(vocab_pathname)):
    vocab = read_vocab_file(vocab_pathname)
else:
    text_ds_vocab = tf.data.Dataset.from_tensor_slices(comibned_text)
...
# Tokenize data and convert back to numpy arrays 
# (because, for some reason, it works better than training with TF datasets)
#XBOOK_dev, YBOOK_dev, CBOOK_dev = format_training_data(booktext_ds_dev,book_ages_dev,en_tokenizer,maxlen)
#XBOOK_train, YBOOK_train, CBOOK_train_ = format_training_data(booktext_ds,book_ages_train,en_tokenizer,maxlen)


# Combined datasets
X_dev_combined,Y_dev_combined, C_dev_combined_ = format_training_data(combined_text_dev,combined_age_dev,en_tokenizer,maxlen)
X_train_combined,Y_train_combined,C_train_combined_ = format_training_data(comibned_text,combined_age_train,en_tokenizer,maxlen)


# saving indices for both datasets into different variables
#valid_inds_book,b = np.where(C_train_combined_ != out_of_range_age)
#valid_inds_childes,b = np.where(C_train_combined_ == out_of_range_age)   


D_trained_combined_ = format_d(C_train_combined_)
D_dev_combined_ = format_d(C_dev_combined_)

# Assign different values of age to C_train and C_train_combined_ whenever the values is out of range age                                                                      
#C_train_combined = replace_values(C_train_combined_, age_groups)
age_groups_ = age_groups + [86]
weight, bin_edges = np.histogram(C_train, bins=age_groups_) #, density=True
C_train_combined = assign_random_values(C_train_combined_, weight)

# Assign different dev age to C_dev_combined_ whenever the values is out of range age                                                                      
C_dev_combined = replace_values(C_dev_combined_)

D_trained_combined = one_hot_encode(D_trained_combined_)
D_dev_combined = one_hot_encode(D_dev_combined_)
...
if(train_model):
    history = model.fit([C_train_combined, D_trained_combined, X_train_combined], Y_train_combined,validation_data=([C_dev_combined, D_dev_combined, X_dev_combined], Y_dev_combined), batch_size=batch_size, shuffle=True, verbose=1, epochs=max_epochs, callbacks=[text_gen_callback,tensorboard_callback, earlystop_callback, model_checkpoint_callback])
...
for age_cond in ages_to_synth:    
    tf.keras.backend.clear_session()
    if(generate_book):
        valid_inds,b = np.where(C_train_combined_ > age_groups[-1]) # &(C_train_combined == age_cond)
    else:
        valid_inds,b = np.where(C_train_combined_ > age_groups[-1]) #np.where(C_train_combined_ > age_groups[-1]) generate by out of domain seed np.where(C_train_combined_ != out_of_range_age)
    for r in range(0,samples_to_generate):        
        txt =""
        while txt.strip()=="":
            element = X_train_combined[valid_inds[np.random.randint(len(valid_inds))],:]
            ...
            txt = sample_new(start_tokens[i:i+start_length].tolist(),tokens_to_generate_per_sampling,en_tokenizer,generate_book,sampling_temperature,np.array(age_cond)) # generate_book 0 for childes style, 1 for book
        write_output_file(maindir + '/txt_out/' + txt_out_name  + '/' + txt_out_name + modelname + '_{}k'.format(sampling_k) + '_{}t'.format(int(sampling_temperature*100)) + '_{:03d}'.format(age_cond)  + '.txt', txt)
        if(np.mod(r,200) == 0):
            tf.keras.backend.clear_session()
