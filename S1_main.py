...
model_basename = 'Styled_S1_' # name of the "experiment", used for saving the models/results
txt_out_name = 'S1_PB30_'  # nametag to add to each of the generated .txt files  Styled_S2_OODseed_20_ Styled_S1_gbook_4_
...
generate_book = 0                       # generate book
ODDseed = 0                             # to use out of domain seed
...
def assign_random_values_(target_array, weight):
    if not age_groups:
        raise ValueError("age groups cannot be empty.")
    
    result = [random.choices(age_groups, weights =weight)[0] if val > age_groups[-1] else val
...
book_ages_dev = [book_dev_age_bin]*len(booktext_ds_dev)
...
vocab_pathname = maindir + 'token_vocabularies/en_vocab_ds_s1_{}.txt'.format(vocab_size)
...
# Combined datasets
X_dev_combined,Y_dev_combined,C_dev_combined = format_training_data(combined_text_dev,combined_age_dev,en_tokenizer,maxlen)
X_train_combined,Y_train_combined,C_train_combined = format_training_data(comibned_text,combined_age_train,en_tokenizer,maxlen)
...
for age_cond in ages_to_synth:    
    tf.keras.backend.clear_session()
    if(generate_book or ODDseed):
        valid_inds,b = np.where(C_train_combined == out_of_range_age) 
    else:
        valid_inds,b = np.where(C_train_combined != out_of_range_age) # == out_of_range_age for ODD seed CDS, normal CDS: != out_of_range_age
    for r in range(0,samples_to_generate):        
        ...
        while txt.strip()=="":
            ...
            txt = sample_new(start_tokens[i:i+start_length].tolist(),tokens_to_generate_per_sampling,en_tokenizer,sampling_temperature,np.array(age_cond)) #age_cond or out_of_range_age in last element for book generation
